...
const cssWClass = new Set([`${prefix}w-m0`, `${prefix}w-p0`]);
cssWidth.forEach(fr => {
  cssWClass.add(`${prefix}w-n${fr.split('/')[0]}d${fr.split('/')[1]}`);
});
for(let ab = 5; ab <= maxSize; ab += 5) {
  cssWClass.add(`${prefix}w-p${ab}`);
  cssWidth.forEach(fr => {
    cssWClass.add(`${prefix}w-n${fr.split('/')[0]}d${fr.split('/')[1]}-m${ab}`);
  });
}
